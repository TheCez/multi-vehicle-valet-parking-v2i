...
def solve_conflict(conflict_area):
    ...
    if start_path_finding:
        ...
        # conflict_area[conflict_area == -4] = 5
        # conflict_area[conflict_area == 6] = 5
        if waypoint is not None:
            min_row, min_col = bounds[0]
            max_row, max_col = bounds[1]
            sub_area = conflict_area[min_row:max_row+1, min_col:max_col+1]
            six_indices = np.argwhere(sub_area == 6)
            sub_area = np.where((sub_area == 5) | (sub_area == -4) | (sub_area == 6), 0, 1)
            conflict_area[min_row:max_row+1, min_col:max_col+1] = sub_area

            for idx in six_indices:
                sub_r, sub_c = idx
                conflict_area[min_row + sub_r, min_col + sub_c] = 6
            # conflict_area[conflict_area == -4] = 0
            waypoint = (waypoint[0], waypoint[1])
            # conflict_area[waypoint[0], waypoint[1]] = 4
            #waypoint = None
        
    return conflict_area , waypoint if start_path_finding else None, stop, waypoint
