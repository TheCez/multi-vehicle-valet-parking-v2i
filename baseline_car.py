    ...
    try:
        ...
        while True:
            ...
            if subscriber.receive_messages():
                ...
                # baseline_occupancygrid, car_box_index = occupationgrid.generate_occupation_grid_baseline(world.player, other_vehicles=other_cars)
                
                # os.makedirs("baseline_grid", exist_ok=True)
                # np.save(os.path.join("baseline_grid", "baseline_grid.npy"), baseline_occupancygrid)
                
                # Process Qt events without blocking
                # QApplication.processEvents()
                print("Hybrid A* started!")
                # time.sleep(3)
                print("Hybrid A* ended!")
