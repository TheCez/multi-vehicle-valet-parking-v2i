                    ...
                    if msg_type == b"ACK":
                        ...
                        if self.conflict_counter == 0:
                            ...
                            if conflict_mask.any():
                                ...
                                decision_grids_copy = {k: v.copy() for k, v in decision_grids.items()}
                                ...
                                if self.decision_to_make:
                                    ...
                                    for subscriber_id, grid in decision_grids.items():
                                        ...
                                        self.subscribers_data[subscriber_id]['direction'] = direction
                                        ...
                                        if check_fit:
                                            ...
                                            self.subscribers_data[subscriber_id].update( {
                                            'car_value': car,
                                ...
                                # Crop the decision grid using the significant mask to get the conflict area

                                all_ids = list(self.subscribers_data.keys())

                                for subscriber_id,data in self.subscribers_data.items():
                                    print(f"SubscriberID {subscriber_id} direction: {data['direction']}")
                                    if data['direction']  == 'R':
                                        decision_grid_copy = decision_grids_copy[subscriber_id].copy()
                                        conflict_area_from_decision = decision_grid_copy[min_r:max_r+1, min_c:max_c+1]
                                        conflict_area_from_decision = np.rot90(conflict_area_from_decision, 2)
                                        if not os.path.exists("waypoint_finder_grid"):
                                            os.makedirs("waypoint_finder_grid")
                                        np.save(f"waypoint_finder_grid/decision_grid_copy_{subscriber_id}.npy", conflict_area_from_decision)
                                        conflict_area_from_decision, new_path_point_decision, waypoint_og_decision = solve_conflict(conflict_area_from_decision)
                                        conflict_area_from_decision = np.rot90(conflict_area_from_decision, 2)
                                        indices_100 = np.argwhere(conflict_area_from_decision == 100)
                                        if indices_100.size > 0:
                                            new_path_point_decision = tuple(indices_100[0])
                                        else:
                                            new_path_point_decision = None
                                        self.subscribers_data[subscriber_id]['waypoint'] = new_path_point_decision
                                        # try:
                                        #     for other_id in all_ids:
                                        #         if other_id != subscriber_id:
                                        #             self.subscribers_data[other_id]['waypoint'] = new_path_point_decision
                                        # except Exception as e:
                                        #     print(f"Error setting waypoint for other_id: {e}")
                                        # conflict_area = self.decision_maker_helper(conflict_area, direction='L')
                                        ...
                                        # except Exception as e:
                                        #     print(f"Error setting waypoint for other_id: {e}")
                                        # conflict_area = self.decision_maker_helper(conflict_area, direction='R')


                                    print(f"waypoint of subscriberID {subscriber_id}: {self.subscribers_data[subscriber_id]['waypoint']}")
                                ...
                                for sid in occupancy_grids:
                                    ...
                                    # Get new_path_point for this sid from decision_grids if available
                                    new_path_point = None
                                    if sid in self.subscribers_data and 'waypoint' in self.subscribers_data[sid]:
                                        new_path_point = self.subscribers_data[sid]['waypoint']
