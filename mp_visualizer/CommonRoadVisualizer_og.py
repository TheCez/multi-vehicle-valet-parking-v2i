...
from commonroad_reach.utility.visualization import generate_default_drawing_parameters, draw_reachable_sets, compute_plot_limits_from_reachable_sets, draw_drivable_area, draw_driving_corridor_2d
...
class CommonRoadVisualizer(QMainWindow):
    ...
    def update_visualization(self, other_cars=None):
        ...
        try:
            ...
            if reachability_results.get("main"):
                try:
                    polygons = self.draw_reachable_area(self.base_config.planning.steps_computation, reachability_results["main"], color="OrRd_d")
                except Exception as e:
    ...
    def draw_reachable_area(self, current_step, reach_interface=None, color="GnBu_d", find_corridor=False):
        ...
        if self.visualize:
            ...
            if find_corridor:
                list_corridor = reach_interface.extract_driving_corridors()
                if list_corridor:
                    draw_params.shape.facecolor = (0, 1, 0, 0.5)
                    draw_params.shape.edgecolor = (0, 0.5, 0, 1)
                    for idx, corridor in enumerate(list_corridor):
                        draw_driving_corridor_2d(corridor, 
                                            dc_id=idx,  # Add this parameter
                                            reach_interface=reach_interface,
                                            rnd=self.canvas.mp_renderer)
