    ...
    try:
        ...
        while True:
            ...
            if subscriber.receive_messages():
                ...
                reach_occupancygrid, car_box_index = occupationgrid.generate_occupation_grid_baseline(world.player, other_cars)
                reach_occupancygrid = reach_occupancygrid.copy().astype(np.int8)

                # # Display the reach occupancy grid using matplotlib
                # import matplotlib.pyplot as plt

                # plt.figure("Reach Occupancy Grid")
                # plt.imshow(reach_occupancygrid, cmap='gray', origin='lower')
                # plt.title("Reach Occupancy Grid")
                # plt.xlabel("X")
                # plt.ylabel("Y")
                # plt.pause(0.001)
                # plt.clf()
